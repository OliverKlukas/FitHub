...
function ContentListViewTest(props) {
    ...
    const onAddContent = () => {
        props.dispatch(props.addContent({
            title: "Test Title"
        }));
    ...
    return !contents ? (// if no movies are loaded, the above useEffect should be triggered
        <Typography>Loading
            contents</Typography>) : !Array.isArray(contents) ? (// apperantly something went wrong, usually there should be some kind of error handling
        <div>error</div>) : (
        <div>
            ...
            <StandardButton variant="contained" onClick={onAddContent}>Add</StandardButton>
        </div>);
}
