    ...
    <Autocomplete
      ...
      onOpen={() =>
        setLastFilter((lastFilter) =>
          lastFilter.filter((tag) => globalFilter.includes(tag))
        )
      }
      onChange={(event, newFilter) => {
        // Check if elements have been deleted and if so delete them in the common filter.
        const add = newFilter.filter((tag) => !lastFilter.includes(tag));
        if (add.length > 0) {
          addFilter(add);
        }

        // Check if elements have been added and if so add them to the common filter.
        const del = lastFilter.filter((tag) => !newFilter.includes(tag));
        if (del.length > 0) {
          deleteFilter(del);
        }
