    ...
    return (<Stack direction="row" justifyContent="space-between" alignItems="center" my={2}>
        <Box>
            ...
            <Box sx={{display: {xs: 'flex', md: 'none'}}}>
                ...
                >
                    {Object.keys(pages).map((url) => (<MenuItem key={url} onClick={handleCloseNavMenu}>
        ...
        <Box>
            ...
            <Stack direction="row" spacing={4} pr={16} sx={{display: {xs: 'none', md: 'flex'}}}>
                {Object.keys(pages).map((url) => (<LinkButton
                        key={url}
                        variant='text'
        ...
        <Box>
            ...
            >
                {Object.keys(settings).map((url) => (<MenuItem key={url} onClick={handleCloseSetMenu}>
                    <Link href={url} underline="none" color="inherit">
                        {settings[url]}
                    </Link>
                </MenuItem>))}
