    ...
    const getNavigationLinks = () => {
        if (user.user) {
            if (user.user.role === "customer") {
                return [["discovery", "Discovery"], ["plans", "My Content"], ["about", "About Us"]];
            } else {
                return [["discovery", "Discovery"], ["dashboard", "My Content"], ["about", "About Us"]];

            }
        }
        return [["discovery", "Discovery"], ["signin", "Sign In"], ["about", "About Us"]];
    }

    // Sub-urls and link names for available user menu options when the user is not logged in
    const settings = {
        contact: "Contact", signin: "Sign in", signup: "Sign up",
    };
    ...
    return (<Stack direction="row" justifyContent="space-between" alignItems="center" my={2}>
        <Box key={"left-header-part"}>
            <Box key={"md-logo"} sx={{display: {xs: "none", md: "flex"}}}>
                <Link
                    href={user.user ? [user.user.role === "contentCreator" ? "/landing" : "/discovery"] : "/discovery"}>
                    <Logo/>
                </Link>
            </Box>
            <Box key={"xs-nav-menu"} sx={{display: {xs: "flex", md: "none"}}}>
                <IconButton size="large" aria-label="account of current user" aria-controls="menu-appbar"
                            aria-haspopup="true" onClick={(event) => setAnchorNavigation(event.currentTarget)}
                            color="inherit">
                    <MenuIcon/>
                </IconButton>
                <Menu
        ...
        <Box key={"middle-header-part"}>
            ...
            <Stack direction="row" spacing={4} pr={16} sx={{display: {xs: "none", md: "flex"}}}>
                {getNavigationLinks().map((item, index) => (
                    <LinkButton key={index} variant="text" component={RouterLink} to={item[0]}>
                        {item[1]}
                    </LinkButton>))}
            </Stack>
        </Box>
        <Stack direction={"row"} alignItems={"center"} key={"right-header-part"}>
            {user.user && <Box>
                <IconButton onClick={(event) => setAnchorNotifications(event.currentTarget)}>
                    <Badge
                ...
                >
                    <MenuList id="notifications-menu" aria-labelledby="notification-button">
                        {newMessages > 0 ? (<MenuItem key="messages" onClick={() => setAnchorNotifications(null)}>
                        ...
                        {newReviews > 0 ? (<MenuItem key="reviews" onClick={() => setAnchorNotifications(null)}>
                            <Link href={`/profile/${id}`} underline="none" color="inherit">
            ...
            >
                {user.user ? <MenuList id="composition-menu" aria-labelledby="composition-button">
                    ...
                    {Object.keys(settings).map((url) => (<MenuItem key={url} onClick={() => setAnchorSettings(null)}>
